...
fabio_footprint <- function(wd, years, regions_select = "all"){
  ...
  footprint <- function(region, fd, select_items = "all", file_name){
    ...
    data.table::fwrite(FP, file=paste0("./results/",year,"_",file_name,".csv"), append = TRUE)
  ...
  for(year in years){
    ...
    headings <- data.table(From.Country.Code = integer(),
                           ...
                           Country = character(),
                           ISO = character())
    data.table::fwrite(headings, file=paste0("./results/",year,"_",file_name,".csv"))
    ...
    FP <- data.table::fread(file = paste0("./results/",year,"_",file_name,".csv"))
    FP <- reshape2::dcast(FP, ... ~ fd, value.var = "value", fun.aggregate = sum)
    data.table::fwrite(FP, file=paste0("./results/",year,"_",file_name,".csv"))
  }
