...
#' 
#' @description Build MRIO table based on Faostat commodity balance sheets and trade data part 1b: prepare CBS and BTD data sets
#' 
#' @param ... some argument 
#' 
...
#' 
#' @export
FAO_MRIO_1b_data_preparation <- function(...){
  ...
  reg <- utils::read.csv(file="Regions_all.csv", header=TRUE, sep=";")
  ...
  for(year in 1986:2013){
    ...
    # aggregate
    ForTrade <- stats::aggregate(Value ~ From.Country.Code + From.Country + To.Country.Code + To.Country + Item.Code + Item + 
                            Year, ForTrade, sum)
    ForTrade$Reporter <- ""
    ForTrade$tHead <- 0
    ForTrade$tUSD <- 0
    ...
    for(item in unique(BTD$Item)){
      if(nrow(BTD[BTD$tonnes > 0 & BTD$tUSD > 0 & BTD$Item==item,])>0){
        Price <- stats::aggregate(cbind(tUSD, tonnes) ~ Year, BTD[BTD$tonnes > 0 & BTD$tUSD > 0 & BTD$Item==item,], sum)
        ...
        Price <- stats::aggregate(cbind(tUSD, tHead) ~ Year, BTD[BTD$tHead > 0 & BTD$tUSD > 0 & BTD$Item==item,], sum)
    ...
    if(year<1995){
      ...
      # calculate prices
      Price <- stats::aggregate(cbind(usd, litres, kg) ~ Year, BTDEthanol[BTDEthanol$kg > 0 & BTDEthanol$usd > 0,], sum)
      ...
      Price <- stats::aggregate(cbind(tUSD, tonnes) ~ Year, BTDEthanol[!is.na(BTDEthanol$tonnes) & BTDEthanol$tUSD > 0,], sum)
      Price$perkg <- Price$tUSD / Price$tonnes # USD per kg
    ...
    if(year<1995){
      ...
      Price <- stats::aggregate(cbind(usd, kg) ~ Year, BTDFish[BTDFish$kg > 0 & BTDFish$usd > 0,], sum)
      Price$perkg <- Price$usd / Price$kg # USD per kg
      # estimate missing kg and l values based on prices
      ...
      # estimate missing kg values based on prices
      Price <- stats::aggregate(cbind(tUSD, tonnes) ~ Year, BTDFish[!is.na(BTDFish$tonnes) & BTDFish$tUSD > 0,], sum)
      Price$perkg <- Price$tUSD / Price$tonnes # USD per kg
    ...
    temp <- stats::aggregate(tonnes ~ To.Country.Code + To.Country + Item.Code + Item + Year, BTD[BTD$Item.Code %in% addCBS$Item.Code,], sum)
    ...
    temp <- stats::aggregate(tonnes ~ From.Country.Code + From.Country + Item.Code + Item + Year, BTD[BTD$Item.Code %in% addCBS$Item.Code,], sum)
    temp <- temp[!temp$tonnes==0,]
    ...
    # livestock imports
    temp <- stats::aggregate(tHead ~ To.Country.Code + To.Country + Item.Code + Item + Year, BTD[BTD$Item.Code %in% addCBS$Item.Code,], sum)
    ...
    # livestock exports
    temp <- stats::aggregate(tHead ~ From.Country.Code + From.Country + Item.Code + Item + Year, BTD[BTD$Item.Code %in% addCBS$Item.Code,], sum)
    temp <- temp[!temp$tHead==0,]
    temp$ID <- paste(temp$From.Country.Code, temp$Item.Code, temp$Year)
    for(id in temp$ID){
    ...
    exports <- stats::aggregate(tonnes ~ From.Country.Code + From.Country, BTD[BTD$Item.Code==2659,c(1,2,10)], sum)
    imports <- stats::aggregate(tonnes ~ To.Country.Code + To.Country, BTD[BTD$Item.Code==2659,c(3,4,10)], sum)
