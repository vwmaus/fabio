#' @title FAO_MRIO_1c_estimate_missing_CBS
#' @author Sebastian Luckeneder, \email{sebastian.luckeneder@@wu.ac.at}
#' 
#' @description Build MRIO table based on Faostat commodity balance sheets and trade data part 1c: estimate missing CBS
#' 
#' @param ... some argument 
#' 
...
#' 
#' @export
FAO_MRIO_1c_estimate_missing_CBS <- function(...){
  ...
  for(year in 1986:2013){
    ...
    for(country in unique(c(unique(Prod_crop_all$Country.Code), unique(Prod_lvst_all$Country.Code)))){
      ...
      # TCF <- utils::read.table(file = "TCF_prod_lvst.csv", sep=";", header = T)
      ...
      # if(nrow(Prod_lvst)>0)
      #   Prod_lvst <- stats::aggregate(Value ~ ., data=Prod_lvst, sum)
      #---#---#---#---#---#---#---#---#---#---#---#---#---#---#---#---
      # prepare CBS
      #---#---#---#---#---#---#---#---#---#---#---#---#---#---#---#---
      ...
      if(country %in% (Prod_crop_all$Country.Code) & nrow(Prod)>0){
        ...
        #---#---#---#---#---#---#---#---#---#---#---#---#---#---#---#---
        TCF <- utils::read.table(file = "TCF_prod.csv", sep = ";", header = T)
        TCF$Processing <- 0
        TCF$Production <- 0
        # item = "Beer"
      ...
      if(nrow(BTD[BTD$Item.Code %in% addCBS$Item.Code & BTD$To.Country.Code==country,])>0){
        imports <- stats::aggregate(tonnes ~ To.Country.Code + To.Country + Item.Code + Item + Year, BTD[BTD$Item.Code %in% addCBS$Item.Code & BTD$To.Country.Code==country,], sum)
        imports <- imports[!imports$tonnes==0,]
        for(item in imports$Item){
          addCBS$Imports[addCBS$Item==item] <- imports$tonnes[imports$Item==item]
        }
        # livestock imports
        temp <- stats::aggregate(tHead ~ To.Country.Code + To.Country + Item.Code + Item + Year, BTD[BTD$Item.Code %in% addCBS$Item.Code & BTD$To.Country.Code==country,], sum)
      ...
      if(nrow(BTD[BTD$Item.Code %in% addCBS$Item.Code & BTD$From.Country.Code==country,])>0){
        exports <- stats::aggregate(tonnes ~ From.Country.Code + From.Country + Item.Code + Item + Year, BTD[BTD$Item.Code %in% addCBS$Item.Code & BTD$From.Country.Code==country,], sum)
        exports <- exports[!exports$tonnes==0,]
        for(item in exports$Item){
          addCBS$Exports[addCBS$Item==item] <- exports$tonnes[exports$Item==item]
        }
        # livestock exports
        temp <- stats::aggregate(tHead ~ From.Country.Code + From.Country + Item.Code + Item + Year, BTD[BTD$Item.Code %in% addCBS$Item.Code & BTD$From.Country.Code==country,], sum)
        temp <- temp[!temp$tHead==0,]
