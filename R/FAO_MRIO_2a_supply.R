#' @title FAO_MRIO_2a_supply
#' @author Sebastian Luckeneder, \email{sebastian.luckeneder@@wu.ac.at}
#'  
#' @description Build MRIO table based on Faostat commodity balance sheets and trade data part 2a: build supply tables
#' 
#' @param ... some argument 
#' 
...
FAO_MRIO_2a_supply <- function(...){
  ...
  share_items <- utils::read.csv(file="Items_supply-shares.csv", header=TRUE, sep=";")
  ...
  for(year in 1986:2013){
    ...
    sup <- utils::read.csv(file="Items_supply.csv", header=TRUE, sep=";")
