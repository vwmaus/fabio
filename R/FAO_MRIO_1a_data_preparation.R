#' @title FAO_MRIO_1a_data_preparation
#' @author Sebastian Luckeneder, \email{sebastian.luckeneder@@wu.ac.at}
#' 
#' @description Build MRIO table based on Faostat commodity balance sheets and trade data part 1a: prepare raw FAO data
#' 
#' @param ... some argument 
#' 
...
#' 
#' @export
FAO_MRIO_1a_data_preparation <- function(...){
  
  rm(list=ls()); gc()
  
  regions <- utils::read.csv2(file = "Regions_all.csv")
  years <- 1986:2013
  ...
  #-----------------------------
  TCF <- utils::read.csv(file="TCF_Trade.csv", header=TRUE, sep=";")
  ...
  #-----------------------------
  Items <- utils::read.csv(file="Items_BTD-CBS.csv", header=TRUE, sep=";")[,1:3]
  ...
  BTD <- data.frame(data.table::as.data.table(BTD)[,list(val = sum(Value)), by = 'Reporter.Country.Code,Reporter.Countries,Partner.Country.Code,Partner.Countries,Item.Code,Item,Year,Imex,Unit'])
  names(BTD)[10] <- "Value"
  #-----------------------------
  ...
  BTD <- as.data.frame(dcast(data.table::as.data.table(BTD), Reporter.Country.Code + Reporter.Countries + Partner.Country.Code + Partner.Countries + 
  ...
  cropcom <- utils::read.csv2(file="Items_Prod-CBS.csv")
  ...
  Fodder <- stats::aggregate(Value ~ ., Fodder[,c(2,3,4,5,7,8,9,10)],sum)
  Fodder$Unit <- as.factor("tonnes")
  ...
  # convert to CBS item codes and aggregate Fodder crops
  cropcom <- utils::read.csv2(file="Items_Proc-CBS.csv")
  # merg data.tables -> much faster than merging data.frames
  # Prod <- merge(Prod, cropcom[,c(1,3,4)], by="Prod.Code", all.x=TRUE)
  ...
  # reg <- utils::read.csv(file="Regions.csv", sep=";")
  ...
  # regfit <- utils::read.csv(file="Regions_Comtrade-FAO.csv", sep=";")
  regfit <- utils::read.csv(file="Regions_ISO3Comtrade-FAO.csv", sep=";")
  ...
  regfit <- utils::read.csv(file="Regions_BACI-FAO.csv", sep=";")
  ...
  regfit <- utils::read.csv(file="Regions_FISH-FAO.csv", sep=";")
  # reg <- utils::read.csv(file="Regions.csv", sep=";")
  FishProd <- merge(FishProd_raw, regfit)
  FishProd <- FishProd[,c(8,9,5,6,3)]
  FishProd <- stats::aggregate(Quantity ~ ., FishProd, sum)
