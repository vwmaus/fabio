#' @title FAO_MRIO_4c_footprints_AB
#' @author Sebastian Luckeneder, \email{sebastian.luckeneder@@wu.ac.at}
#' 
#' @description Build MRIO table based on Faostat commodity balance sheets and trade data part 4c: calculate footprints AB (?)
#' 
#' @param ... some argument 
#' 
...
#' 
#' @export
FAO_MRIO_4c_footprints_AB <- function(...){
  ...
  for(year in 1986:2013){
    ...
    MP_A <- data.table::setDT(MP)[, lapply(.SD, sum), by = .(ID)]
    MP_A$ID <- NULL
    MP_A <- as.matrix(MP_A)
    ...
    MP_B <- data.table::setDT(MP)[, lapply(.SD, sum), by = .(ID)]
    MP_B$ID <- NULL
    MP_B <- as.matrix(MP_B)
    ...
    Y$ISO <- NULL
    Y <- stats::aggregate(. ~ Continent + FD, Y, sum)  # 164 sec
    # Y <- as.data.frame(data.table::setDT(Y)[, lapply(.SD, sum), by = .(Continent,FD)])  # 300 sec
    # Y <- as.data.frame(data.table::setDT(Y)[, lapply(.SD, sum), by = "Continent,FD"])  # 244 sec
    # Y <- as.data.frame(Y %>% dplyr::group_by(Continent, FD) %>% dplyr::summarise_all(sum))  # 607 sec
    ...
    for(region in regions_selected$Nr){
      ...
      for(fd in 1:4){
        ...
        FP <- data.table::as.data.table((t(MP_A) * Yreg))
        ...
        FP <- data.table::as.data.table((t(MP_B) * Yreg))
        FP$ID <- items$Com.Code
        FP <- as.data.frame(FP[, lapply(.SD, sum), by = .(ID)])
        FP$ID <- NULL
        FP <- t(FP)
    ...
    for(region in regions_selected$Nr){
      ...
      # fd = 1
      for(fd in 1:4){
        f <- data.table::as.data.table(FP_region[[region]][[fd]][["A"]])
    ...
    FP$Country.Nr <- NULL
    data.table::fwrite(FP, file=paste0("results/Biofuels/",year,"_FP_selected_A.csv"), sep=";")
    ...
    for(region in regions_selected$Nr){
      ...
      # fd = 1
      for(fd in 1:4){
        f <- data.table::as.data.table(FP_region[[region]][[fd]][["B"]])
        colnames(f) <- as.character(items$Com.Code)
    ...
    FP$ISO <- regions_selected$ISO[match(FP$Country.Nr,regions_selected$Nr)]
    FP$Country.Nr <- NULL
    data.table::fwrite(FP, file=paste0("results/Biofuels/",year,"_FP_selected_B.csv"), sep=";")
