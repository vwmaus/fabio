#' @title FAO_MRIO_1d_consolidate_CBS_and_BTD
#' @author Sebastian Luckeneder, \email{sebastian.luckeneder@@wu.ac.at}
#' 
#' @description Build MRIO table based on Faostat commodity balance sheets and trade data part 1d: consolidate CBS and BTD
#' 
#' @param ... some argument 
#' 
...
#' 
#' @export
FAO_MRIO_1d_consolidate_CBS_and_BTD <- function(...){
  ...
  for(year in 1986:2013){
    ...
    temp$Country <- "RoW"
    temp <- stats::aggregate(. ~ Country.Code + Country + Item.Code + Item + Year, temp, sum)
    temp$Original <- 0
    ...
    BTD <- data.table::as.data.table(BTD)
